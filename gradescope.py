...
def submit_assignment(file_name,assignment_number,class_number):
    ...
    if class_number == "2":
        ...
        new_num = int(assignment_number)
        new_num += 1
        
        with open('hw_num.txt','w') as file:
...
while True:
    ...
    if len(added2) != 0:
        file_name = added2[0]
        """This is the implimentation of an interesting work-around.
        For this class, the assignments aren't named after the specific homework
        assignment but rather based on a numbering system starting at 1, 
        going up with each assignment. The text file referenced here ('hw_num.txt') 
        keeps track of which homework assignment I'm on and increases the count
        by one every time I submit an assignment. It's not foolproof but
        works for the time being.
        """
        with open('hw_num.txt','r') as file:
            num = file.read()

        submit_assignment(file_name,num,'2')
